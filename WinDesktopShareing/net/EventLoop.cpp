...
TimerId EventLoop::AddTimer(TimerEvent timer_event, uint32_t msec)
{
    std::lock_guard<std::mutex> locker(mutex_);
    if (task_schedulers_.size() > 0) {
        return task_schedulers_[0]->AddTimer(timer_event, msec);
    }
    return 0;
}
...
void EventLoop::RemoveTimer(TimerId timer_id)
{
    std::lock_guard<std::mutex> locker(mutex_);
    if (task_schedulers_.size() > 0) {
        task_schedulers_[0]->RemoveTimer(timer_id);
    }
}
